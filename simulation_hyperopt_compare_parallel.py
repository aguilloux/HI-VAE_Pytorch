...
def run(generator_name, method_hyperopt):
    ...
    # Save the data
    dataset_name = "Compare_hyperopt_methods"
    ...
    study_name = original_dir + "/dataset/" + dataset_name + "/optuna_results/optuna_study_{}_ntrials{}_{}_{}_{}".format(name_config, n_trials, metric_optuna, method_hyperopt, generator_name)
    best_params_file = original_dir + "/dataset/" + dataset_name + "/optuna_results/best_params_{}_ntrials{}_{}_{}_{}.json".format(name_config, n_trials, metric_optuna, method_hyperopt, generator_name)
...
if __name__ == "__main__":
    ...
    if id < 0 or id >= len(combinations):
        raise ValueError(f"Invalid ID: {id}. Must be between 0 and {len(combinations)-1}")
    gen, method = combinations[id]
    run(gen, method)