...
def true_univ_coef(treatment_effect_list, independent = True, surv_type = 'surv_piecewise', 
               feature_types_list = ["pos", "real", "cat"], n_features_bytype = 4, n_active_features = 3 , p_treated = 0.5, 
               shape_T = 2, shape_C = 2, scale_C = 6., scale_C_indep = 2.5, data_types_create = True, seed=0):
    # Compute univariate treatment effects
    n_samples = 100000
    coef_init_univ = []
    for treat_effect in treatment_effect_list:
        ...
        control, treated, types = simulation(treat_effect, n_samples,
                                    independent = independent,
                                     ...
                                     surv_type = surv_type,
                                     shape_T = shape_T , shape_C = shape_C , scale_C = scale_C , scale_C_indep = scale_C_indep, seed=seed)
    
        df_init = pd.concat([control, treated], ignore_index=True)
        columns = ['time', 'censor', 'treatment']
        coef_init, _, _, _ = fit_cox_model(df_init, columns)
        coef_init_univ.append(coef_init[0])
    return(np.array(coef_init_univ))
...
def cpower(mc , mi , loghaz , alpha):
    ...
    loghaz : log of hazard ratios or univ. treatment coefficient
    alpha : level of test
    """
    ## Find its variance
    v = 1/mc + 1/mi
