...
def plot_compare_data(data, feat_types_dict,feat_comparison_name):
    """
    Visualize different data types.
    ...
    for i, feature in enumerate(feat_types_dict):
        if feature['name'] == feat_comparison_name:
            ...
            print("comparison variable",feature['name'] ,feat_comparison_index)
    ...
    for i, feature in enumerate(feat_types_dict):
        if feat_idx < feat_comparison_index:
            ...
            if feature_type in ['cat', 'ordinal']:  # Count, ordinal & categorical data
                ...
                # sns.countplot(data=feature_data, x=feat_name, hue=feat_name, palette="Set1", alpha=0.8, legend=False, ax=ax)
                sns.countplot(data=feature_data, x=feat_name, hue=feat_comparison_name, alpha=0.8, legend=False, ax=ax)
                ax.set_title(f"Count plot of {feat_name} ({feature_type})", fontsize=16, fontweight="bold")
                ...
                #print(feature_data.head())
                #sns.histplot(feature_data, kde=False, x=feat_name, hue=feat_comparison_name, color="royalblue",ax=ax)
        ...
        if feat_idx > feat_comparison_index:
            ...
            ax= axes[(i-1) // 2, (i-1) % 2]
            if feature_type in ['cat', 'ordinal']:  # Count, ordinal & categorical data
                feature_data = pd.DataFrame(data[:, [feat_idx+1,feat_comparison_index]], columns=[feat_name,feat_comparison_name])
                #print(feature_data.head())
                # sns.countplot(data=feature_data, x=feat_name, hue=feat_name, palette="Set1", alpha=0.8, legend=False, ax=ax)
                sns.countplot(data=feature_data, x=feat_name, hue=feat_comparison_name, alpha=0.8, legend=False, ax=ax)
                ax.set_title(f"Count plot of {feat_name} ({feature_type})", fontsize=16, fontweight="bold")
...
# def plot_est_data(data, feat_types_dict):
#     """
#     Visualize different data types.
...
#             ax.set_title(f"Distribution plot of {feat_name} ({feature_type})", fontsize=16, fontweight="bold")
#             kmf = KaplanMeierFitter()
#             subset = data[:, feat_idx : feat_idx + 2]
#             time_cens = (torch.min(subset, dim=1, keepdim=True))

#             survival_time, censoring_indicator = time_cens.values,1-(time_cens.indices)
...
def plot_true_vs_estimation(true_values, estimated_values, miss_mask, types_dict, num_sel_samples=100):
    ...
    for i, feature in enumerate(types_dict):
        ...
        if feature_type in ['surv', 'surv_weibull']:  # Continuous & categorical data
            ...
            #est_survival_time = true_values[:, feat_idx].cpu().numpy()
            #test_censoring_indicator = true_values[:, feat_idx + 1].cpu().numpy()
