...
def run(generator_name):
    ...
    db_file = original_dir + "/optuna_results/optuna_study_{}_ntrials{}_{}.db".format(name_config, n_trials, generator_name)
    if os.path.exists(db_file):
        ...
        if generator_name in ["HI-VAE_weibull", "HI-VAE_piecewise"]:
            ...
            best_params, study = generators_dict[generator_name].optuna_hyperparameter_search(df_init_control_encoded,
                                                                                            ...
                                                                                            epochs=1000,
                                                                                            study_name=original_dir + "/optuna_results/optuna_study_{}_ntrials{}_{}".format(name_config, n_trials, generator_name))
            best_params_dict[generator_name] = best_params
            study_dict[generator_name] = study
            with open(original_dir + "/optuna_results/best_params_{}_ntrials{}_{}.json".format(name_config, n_trials, generator_name), "w") as f:
