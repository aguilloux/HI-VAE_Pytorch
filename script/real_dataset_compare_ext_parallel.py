...
import datetime
import uuid
from utils.metrics import general_metrics, replicability_ext, fit_cox_model
...
def run(dataset_name, generators_sel):
    ...
    for generator_name in generators_sel:
        ...
        for f in os.listdir(best_param_dir):
            if (f.endswith(generator_name + '.json') & ("ext_" + name_config in f)):
                best_param_file = f
    ...
    general_scores_df.to_csv(original_dir + "/dataset/" + dataset_name + '/metric_results/general_scores_df_ext.csv', index=False)

    # replicability_scores = []
    # for generator_name in generators_sel:
    #     replicability_scores.append(replicability_ext(df_init_full, df_syn_dict[generator_name], generator_name))
    # replicability_scores_df = pd.concat(replicability_scores, ignore_index=True)
    # replicability_scores_df.to_csv(original_dir + "/dataset/" + dataset_name + '/metric_results/replicability_scores_df_ext.csv', index=False)

    # columns = ['time', 'censor', 'treatment']
    # _, _, ci_init, _ = fit_cox_model(df_init_full, columns)
    ...
    #                       "label" : label,
    #                       "colors" : colors_})
    # err_df.to_csv(original_dir + "/dataset/" + dataset_name + '/metric_results/error_df_ext.csv', index=False)
